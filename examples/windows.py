...
from PySide6.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QWidget, QCheckBox, QSplitter, QLabel, QSlider
...
class RealTimeCrossSectionViewer(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.toggle_line_cb.setChecked(True)  # default ON
    ...
    def update_cross_section(self):
        ...
        try:
            ...
            # Create mask BEFORE clipping — check which points are inside volume
            valid_mask = (x >= 0) & (x <= self.volume.shape[2] - 1) & \
                        (y >= 0) & (y <= self.volume.shape[1] - 1)

            # Prepare safe coordinates for sampling:
            x_safe = np.copy(x)
            y_safe = np.copy(y)

            # Set out-of-bound points to -1 so map_coordinates samples 0 there
            x_safe[~valid_mask] = -1
            y_safe[~valid_mask] = -1
...
if __name__ == "__main__":
    ...
    # Create test data (100 slices of 256x256)
    z_dim, y_dim, x_dim = 100, 256, 256

    # Create a 2D chessboard pattern for one slice
    x = np.arange(x_dim)
    y = np.arange(y_dim)
